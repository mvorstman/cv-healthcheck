...
DATASET_GUID="$1"
ENDPOINT="${CV_BASE_URL%/}/commandcenter/api/cr/reportsplusengine/datasets/${DATASET_GUID}"
...
if ! RESPONSE="$(curl "${CURL_OPTS[@]}" \
  ...
  "${ENDPOINT}")"; then
  echo "ERROR: curl could not reach Reports Plus metadata endpoint" >&2
  exit 1
fi
...
if [[ "${STATUS}" == 2* ]]; then
  echo "SUCCESS: Reports Plus metadata responded with HTTP ${STATUS}"
  printf '%s\n' "${BODY}"
else
  echo "ERROR: Reports Plus metadata probe failed with HTTP ${STATUS}" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi
