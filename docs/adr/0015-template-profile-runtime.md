...
1. **Cross-environment id variance is assumed but not yet demonstrated** — the repo holds one environment; verified the moment the Phase-1 two-customer lab runs the same template against a second environment. Until then the burden of proof stays open. *Progress note (2026-06-14):* the Fix-4 session-level wire-CCID comparand (live CC-API collects now probe the session's CommServ identity once and verify it against the declared CommCell ID — see CHANGELOG) makes **declared-vs-wire id variance directly OBSERVABLE on the generic collect path**, not just the bespoke environment card. That is a step toward this burden of proof, **not a discharge of it**: it observes variance within one environment; closing the item still requires running the same template against a *second* environment.
