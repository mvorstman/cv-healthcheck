...
**D4a — Rule ownership/classification axis (shipped 2026-06-14; first D4 slice, INERT).** The rules registry gains a `rule_class` column — `policy` (universal) | `customer_assertion` (customer/person/org-specific) — the "template-vs-profile axis" D4 calls for, separating universal policy from profile-layer customer assertions in the previously-undifferentiated namespace. A 2026-06-14 read-only audit classified all 16 live rules (10 policy / 6 customer_assertion) with a clean binary; the migration backfills the customer_assertion set (policy rules take the `policy` default). `save_rule` accepts/validates/preserves `rule_class`; `list_rules`/MCP expose it. **INERT**: classification only — verdict composition, binding, and override resolution are unchanged (the evaluator never reads the column). What it is NOT: not the profile object (D4b — ordered selected templates, scoped customer/project); not the SPCJ resolved-id/parameter portability (D4c — coupled to §119, needs a second live environment); not a `row_rules` move. **D4b flag:** default-policy is safe only while the axis is inert — once scoping consumes it, D4b must decide whether scoping REQUIRES explicit classification rather than defaulting universal (a customer_assertion mis-defaulted to `policy` would fire against every customer).
...
A 2026-06-14 read-only two-customer audit confirmed the **cross-isolation** half (customer A's data under customer B) was already structurally + physically impossible; the only residue was the bounded no-context → **Default** display hazard, which the read-side fix closes. **The Context Integrity read-isolation gate is CLOSED (browser-verified 2026-06-14):** no-context → empty state on the workspace + canonical APIs; HomeLab selected → only HomeLab data; Test_Customer_2 selected → only TC2 data; a TC2 License Summary HTML import landed under the active TC2 context (scoped to the selection, not the file). **D4 (report-profile / bindings ownership) is now unblocked.** Remaining, explicitly NOT part of this gate: §119 cross-environment id-variance (a template-portability question needing a live two-environment **collect**); and the deferred move of active context from the Flask session into `app.db` (the narrow read-fix proved sufficient without it).
...
1. **Cross-environment id variance is assumed but not yet demonstrated** — the repo holds one environment; verified the moment the Phase-1 two-customer lab runs the same template against a second environment. Until then the burden of proof stays open. *Progress note (2026-06-14):* the Fix-4 identity verdict now compares the CommServe **csGUID** (a single stable namespace; live CC-API collects probe `/CommServ` once and verify it against the customer's declared/​TOFU-learned GUID — see CHANGELOG). This corrects the earlier session-wire-CCID comparand, which compared the declared **licensed** CCID against the wire **internal** `commCellId` (different namespaces → false-mismatch). GUID-based identity is exactly the portability direction §119 points to, and it makes **declared-vs-wire id variance directly OBSERVABLE on the generic collect path**. That is a step toward this burden of proof, **not a discharge of it**: it observes variance within one environment (and, by design, TOFU-learns rather than verifies on first contact); closing the item still requires running the same template against a *second* environment.
