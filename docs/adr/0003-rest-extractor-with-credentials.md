...
Investigation of the License Summary report's actual API traffic settled a question the survey couldn't: the **cacheId pattern is what production Commvault uses**. The browser POSTs `reportBuilder.do` once, gets a cacheId, and reuses it across 7+ dataset GETs to render the License Summary page. The "direct GET, no cacheId" path that SA and LS use is the historical outlier — built before the catalog-driven path existed. Each dataset row also comes back with display-ready `_formatted` siblings, meaning the per-subject "normalize" modules are mostly splitting datasets and labeling them, not transforming data.
...
All sections within a single subject must reference the same `report_id`. The `reportBuilder.do` POST happens once per subject collection, the returned `cacheId` is reused across all section fetches for that subject, and the report definition is walked to resolve each section's `dataset_name` to its current `dataset_guid`.
...
2. Opens a `CommvaultSession(base_url, token)` and POSTs `reportBuilder.do` with the subject's `report_id` to obtain the `cacheId` and the live report definition.
3. Loads the subject's section instructions from `subject_section_sources` filtered to `source_type='rest'`.
4. For each section: resolves `dataset_name` → `dataset_guid` from the report definition, calls `session.fetch_dataset(guid, fields, orderby, limit, parameters, …)`, applies generic post-processing (timestamp parsing, null normalization, `_formatted`-sibling handling), and assigns the result to `result.sections[section_id]` with the section's `output_as` mode.
...
Error handling is fail-whole. If any section's fetch fails, the run aborts with a clear error; partial artifacts are not written. The cacheId is acquired once per collection run and reused; if it expires mid-run, the collection fails and is retried fresh.
...
Existing SA/LS artifacts under `data/catalog/artifacts/<customer>/<project>/working/security_assessment/` and `…/license_summary/` are in the **old shape**. ADR 0003 adopts the **new canonical shape** produced by `result_to_artifact` for all subjects, including SA and LS. A one-time forward-migration script converts existing SA/LS `latest.json` and timestamped artifacts to the new shape; downstream code that reads these artifacts is updated to the new shape rather than maintaining dual-read compatibility. Customers that prefer a fresh collection over a migration can simply re-collect once ADR 0003 ships.
...
**Positive.** One extractor replaces three. AI-proposed subjects can use REST without a Python edit. The catalog becomes the single source of truth for what a REST collection does. The cacheId pattern means one `reportBuilder.do` POST per subject collection instead of one per dataset, which matches what production Commvault does and is more efficient for multi-section subjects like License Summary. CommCell URL and provenance derive from the active customer row, making multi-customer collection work correctly. Credentials remain unstored, preserving ADR 0002's decision.

**Negative.** SA and LS artifact shapes change, which requires a one-time forward migration and breaks any out-of-tree code reading those artifacts. The cacheId pattern is more state to manage during a collection (session, cacheId, report definition cache) than the direct-GET pattern. Switching active customers mid-session invalidates the token and forces re-authentication, which is friction the current single-CommCell model doesn't have.
...
**Open questions.** Whether `subject_section_sources` needs a constraint enforcing "all REST sections in a subject share the same `report_id`," or whether that's a runtime check. Whether the cacheId lifetime under load is long enough for a multi-section collection in practice, or whether per-section refresh is occasionally needed (a runtime concern, not an ADR concern, but worth verifying during implementation).
...
- Session: `src/cvhealthcheck/reportsplus/session.py::CommvaultSession` (the cacheId-aware session; the protocol shape ADR 0003 standardizes on).
