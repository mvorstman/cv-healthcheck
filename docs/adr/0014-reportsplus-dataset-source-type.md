...
## Resolved questions (amendments at acceptance, 2026-06-11)

- **Artifact `SourceType` mapping — resolved: `SourceType.rest`.** The extraction type carries the addressing grammar; the artifact source type carries the transport. `reportsplus_dataset` is REST transport, so it maps to `SourceType.rest` (a live source — `collected_at` set). The legacy `reportsplus_rest` enum value remains the retired SA adapter's, untouched.
- **Composite-address form — verified, not open.** The composite form was verified live on **2026-06-06 against CS01**: rows retrieved from `/datasets/{reportGuid}:{componentGuid}/data` for report id 194, with `parameter.timeframe` and `parameter.datasource[]` as query strings. Composite = path segment. The curl-first gate (D5) stands as implementation step 1, but its purpose is **re-confirmation**; a different result is a regression to investigate, not an open design fork. Re-confirmed by the 2026-06-11 gate (`docs/research/adr0014-gate-findings.md`), which additionally established: the composite's second half is the report definition's per-report **entry `guid`** (not the underlying `dataSetGuid` — they differ for some datasets, and the bare `dataSetGuid` 500s for report-bound datasets); bare GUIDs remain valid for standalone datasets; the parameter convention is the generic `parameter.<datasetParamName>` (repeated `name[]` for lists); and unknown parameter names are **silently ignored** by the engine, so the implementation must validate declared parameter names against the dataset's declared parameters (the metadata endpoint `/datasets/{address}` serves both address forms) and fail loudly on mismatch.
