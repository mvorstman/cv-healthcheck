...
| Security Assessment | Can Security Assessment evidence be acquired and normalized consistently across REST, HTML export, and CSV export sources? | Canonical checklist artifact with stable section/check/status/remarks/action fields plus source metadata | API_MAPPING.md -> Security Assessment canonical artifact; Reports Plus Security Assessment report 336; Security Assessment HTML export import; Security Assessment CSV export import | Current validation checks artifact acquisition, canonical field presence, normalization success, and parser resistance to noisy HTML. No health scoring rules are implemented yet. | INFO | IMPLEMENTED | Current phase is acquisition/normalization only. Canonical artifact validation exists, but scoring and final health interpretation do not. Remaining open issue: possible REST/source-precedence or stale artifact selection behavior in the render path. |
