...
| Reports Plus Report 318 metadata | ReportsPlus API | `/commandcenter/api/cr/reportsplusengine/reports/318` | GET | Login-issued Authtoken or Flask session token | None known | LIKELY | Implemented focused extraction path for Report 318 without scraping the Command Center UI route. The extractor persists `data/catalog/reportsplus/report_318_metadata.json` and `report_318_definition.json`, parses both direct `content` definitions and report `pages[].body` JSON definitions, and redirects to login on HTTP 401. A prior live response returned HTTP 200 and showed Report 318 metadata with `pages[].body` and embedded `dataSets`; fresh validation on 2026-05-13 was blocked because the configured lab host was unreachable from this environment. |
| Reports Plus Report 318 dataset map | Local extraction artifact from ReportsPlus API responses | `data/catalog/reportsplus/report_318_dataset_map.json` | Local JSON | Derived from authenticated Reports Plus report and dataset metadata calls | Report definition dataset references | LIKELY | The reusable extractor maps discovered widgets/sections to backing datasets, including nested `dataSet.dataSetGuid`, dataset id/name, required parameters, default parameters, available fields, selected/displayed fields, filters, sorting, limits, and execution endpoint. Backing dataset metadata is fetched with `/commandcenter/api/cr/reportsplusengine/datasets/<datasetGuid>` using the same Login-issued/session token path as report inventory. |
