...
| License Summary canonical artifact | Local canonical artifact shared by REST, XLSX-recorded REST, HTML, and CSV ingestion | `data/catalog/license_summary/latest.json` | Local JSON | Derived from normalized source artifact | Source-specific normalization output | PROTOTYPED | Canonical License Summary artifact with report metadata plus two normalized row collections: `other_licenses[]` and `agent_feature_licenses[]`. The schema is shared across CSV imports, HTML imports, XLSX API viewer recordings, and live REST extraction. |
