...
Current application-state note:

- `data/app.db` is now used for business/application state such as customers and engagements.
- Import registries and canonical artifact storage remain separate from `data/app.db`.
- The new `db/` package uses raw SQL and lightweight schema/migration files; it is not the source of truth for imported HealthCheck artifacts.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CommCell identity/version | Command Center REST API | `/commandcenter/api/CommServ` | GET | Login-issued Authtoken or Flask session token | None known | PROVEN | Live validated 2026-05-14, full field set reconfirmed 2026-05-31 against the captured response (`data/catalog/rest/commserv.json`, http_status 200): root fields `hostName`, `csVersionInfo`, `releaseId`, `osType`, `timeZone` (a `"0:0:<IANA>"` composite), `currentSPVersion`, `installedSPVersion`; nested `commcell.{commCellId, commCellName, csGUID}` and `csTimeZone.{TimeZoneID, TimeZoneName}` (`TimeZoneName` is the clean IANA value). **No license fields are present in this response** (Edition / Mode / Serial / Reg Code / expiry / IPs live elsewhere). The environment / CommCell Details card reads these fields directly from the `raw` response. |
...
| Reports Plus Backup Job Summary dataset | ReportsPlus Dataset | `/commandcenter/api/cr/reportsplusengine/datasets/2638c3d3-adc7-4b61-bb24-2ba509229bf5/data` | GET | Login-issued Authtoken or Flask session token | `format=object`; `includeOther=false`; bounded `limit` | PROTOTYPED | Phase 1 collector foundation only. Uses the validated main dataset GUID `2638c3d3-adc7-4b61-bb24-2ba509229bf5` and tracks related dataset GUID `ce01fc88-d2bd-46cc-ba41-1d967c7fa4a2` for later follow-up. Current implementation normalizes Backup Job Summary rows into `data/catalog/quickhc/backup_job_summary_latest.json` with status breakdown, protected-client count, recent failures, and recent jobs. Quick HC tile rendering is intentionally deferred to Phase 2. |
| License Summary canonical artifact | Local canonical artifact shared by REST, XLSX-recorded REST, HTML, and CSV ingestion | `data/catalog/license_summary/latest.json` | Local JSON | Derived from normalized source artifact | Source-specific normalization output | PROTOTYPED | Canonical License Summary artifact with report metadata plus normalized detail collections `other_licenses[]` and `agent_feature_licenses[]`, and workload/category summary collections in `workload_summary_sections[]`. The schema is shared across CSV imports, HTML imports, XLSX API viewer recordings, and live REST extraction. Missing upstream metadata such as `license_expiry` remains unset rather than guessed. A canonical adapter now exists and canonical side-write is wired for both REST collection and file import. |
...
| License Summary canonical API | Local canonical read API | `/api/license-summary/canonical` | GET | Local app access | None | PROVEN | Returns the current canonical License Summary artifact as JSON for debugging, UI translation layers, and future HealthCheck integration. |
...
| Security Assessment canonical API | Local canonical read API | `/api/security-assessment/canonical` | GET | Local app access | None | PROVEN | Returns the current canonical Security Assessment artifact as JSON for debugging, UI translation layers, and future HealthCheck integration. |
...
| Audit trail retention (`audit_trail` subject) | Command Center REST API (ADR 0009 `rest_command_center_api`) | `/commandcenter/api/commserv/audittrail` | GET | Login-issued Authtoken or Flask session token (app-held; collected app-side via the ADR 0008 internal loopback, no token in the AI/MCP layer) | None | PROVEN | Live-validated 2026-06-04 via the ADR 0008 loopback against the lab CommServe (`192.168.182.129:4433`): HTTP 200, body `{"auditTrailInfo": {...}}` — a single object with `retentionForCritical/High/Medium/Low`. Feeds the `audit_trail` subject's retention table (one row). A re-probe on 2026-06-04 returned HTTP 401 with the current stale token (whole-connection token expiry, not endpoint-specific); retest with a fresh login token. |
| Metrics reporting config (`metrics_reporting` subject) | Command Center REST API (ADR 0009 `rest_command_center_api`) | `/commandcenter/api/commserv/metricsreporting` | GET | Login-issued Authtoken or Flask session token (app-held; collected app-side via the ADR 0008 internal loopback) | None | PROVEN | Live-validated 2026-06-04 via the ADR 0008 loopback (lab CommServe `192.168.182.129:4433`): HTTP 200, body `{"config": {...}}` with metrics package/upload fields and `config.cloud.serviceList` (8 cloud services, each `{enabled, flags, service:{id,name}}`). Feeds the `metrics_reporting` subject (status card + a services table). Re-probe 2026-06-04 returned HTTP 401 with the stale token; retest with a fresh login token. |
| CommServe software cache (`commserve_software_cache` subject) | Command Center REST API (ADR 0009 `rest_command_center_api`) | `/commandcenter/api/commserv/addremovesoftware/commservesoftwarecache` | GET | Login-issued Authtoken or Flask session token (app-held; collected app-side via the ADR 0008 internal loopback) | None | PROVEN | Live-validated 2026-06-04 via the ADR 0008 loopback (lab CommServe `192.168.182.129:4433`): HTTP 200, body `{"commserveSoftwareCache": {...}}` with `cacheFreeSpace`, `UaInfo`, and the nested `UaInfo.cacheContents[].softwareCacheServicePackDetails` (per-platform service-pack rows; WinX64 / linux-x8664 / linux-arm64 at SP 11.40.47). Feeds the `commserve_software_cache` subject (cache configuration + cache contents tables). Re-probe 2026-06-04 returned HTTP 401 with the stale token; retest with a fresh login token. |
| Server groups inventory (`server_groups` subject) | Command Center REST API (ADR 0009 `rest_command_center_api`) | `/commandcenter/api/v4/servergroup` | GET | Login-issued Authtoken or Flask session token (app-held; collected app-side via the ADR 0008 internal loopback) | None | PROVEN | Live-captured 2026-06-04 (token alive this session) via the ADR 0008 loopback against the lab CommServe (`192.168.182.129:4433`): HTTP 200, body `{"serverGroups": [...]}` — a collection of 21 server-group objects, each `{id, name, association, company, description, isSyncInProgress, serverCount}`. Feeds the `server_groups` subject (a table of groups). This is the ADR-0009 end-to-end acceptance-test endpoint (previously declared-but-unvalidated), now confirmed live. |

## Quick HC Integration Notes

- Legacy GET detail pages for Security Assessment and License Summary now redirect to `/quick-hc`.
- POST import and collect handlers remain active and are not redirect shims.
- Quick HC action URLs are now supplied through tile metadata and initial subject data rather than directly from template markup.
