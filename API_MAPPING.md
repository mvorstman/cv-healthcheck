...
| Reports Plus Report Inventory | ReportsPlus API | `/commandcenter/api/cr/reportsplusengine/reports` | GET | Authtoken | None known | LIKELY | Endpoint path implemented for Phase 2 discovery. Lab call reached the endpoint but current token returned `Unauthenticated`; validate with a fresh Reports Plus token before marking proven. Expected records include reportId, userReportId, reportName, description, version, guid, revision, flags, engineVersion, deployed, deployedVersion, viewable, editable, content, type, and isMetrics. |
| Reports Plus Dataset Inventory | ReportsPlus API | `/commandcenter/api/cr/reportsplusengine/datasets` | GET | Authtoken | None known | LIKELY | Endpoint path implemented for Phase 2 discovery. Lab call reached the endpoint but current token returned `Unauthenticated`; validate with a fresh Reports Plus token before marking proven. |
