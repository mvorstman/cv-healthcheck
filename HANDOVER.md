...
- **65-2a** (`c243057`) — **#25 repoint** (repoint-FIRST): `client_growth`/`capacity_license` tile `detail_endpoint` → `main.quick_hc` (registry.py:248,282), matching SA/LS. Guard test: every tile endpoint resolves under app context, none point at a retired dev route. **Backlog #25 RESOLVED.**
- **65-2b** (`70d8a0f`) — **(a) obviated** metric dev pages deleted (`/metrics/client-count|client-growth|capacity-license` + the exclusive `metric_detail.html` + Metric Details landing section).
- **65-2c** (`27101e1`) — **(b) disposable scaffolding** deleted: `/lab-readiness`, `/api/test`, the whole `/reportsplus/*` exploration cluster (11 routes + 11 exclusive templates + now-unused dev-blueprint imports). Landing slimmed; `base.html`/`security_assessment.html` dangling refs fixed.
- **65-2d** (`a3a9d47`) — **(c) staging-preservation guards**: the review-loop endpoints (`/quick-hc/staging` + approve/reject) stay registered, and web + MCP staging share the same `db.staging` backend.

Browser-verified: dev hub slimmed; retired routes 404; the two detail-link repoints open the workspace with no 500; held SA cluster works; `/quick-hc/staging` review loop intact.

**Key points recorded this phase:**
- **The gate corrected the brief's load-bearing premise.** The AI-authoring review loop is the **top-level `/quick-hc/staging` page** (`staging.py` / `main.quick_hc_staging`), NOT in the dev-tools blueprint — so retiring dev tools cannot touch it. The dev **"Security Assessment Registry (internal)"** is a *different* surface (`SecurityAssessmentService.get_history()`, SA artifact-collection history, no staging) → **(b) deletable**, not the (c) the brief feared. Verified web + MCP staging both drive `cvhealthcheck.db.staging`.
- **SA dev cluster HELD for its own pass** (steering decision) — `reportsplus_security_assessment` + import/history/registry-export/registry-view. Biggest blast radius; needs canonical-SA coverage parity check; overlaps backlog #14. The dev blueprint + its remaining `shared.py` orphan helpers get fully reaped when that pass lands.

### Prior: ADR 0004 phase 6 (migrate client_growth) — PASS
...
24. **[MOSTLY DONE — phase 6.5 part 1, commits `c243057`/`70d8a0f`/`27101e1`/`a3a9d47`] Dev tools retirement.** Phase 6.5 part 1 deleted the **(a) auto-obviated** metric pages and the **(b) disposable scaffolding** (env tools + the entire Reports Plus exploration cluster) — 14 routes + their exclusive templates — and repointed #25. **Correction to the load-bearing warning below:** the gate found the brief MISIDENTIFIED the (c) surface. The AI-authoring review loop is the **top-level `/quick-hc/staging` page** (`staging.py` / `main.quick_hc_staging`), which is NOT in the dev-tools blueprint and was never at risk; the dev **"Security Assessment Registry (internal)"** is a *separate* SA artifact-history view (`SecurityAssessmentService.get_history()`, no staging) and is **(b) deletable**. Both web Staging and the MCP staging tools drive the same `cvhealthcheck.db.staging` backend (guarded by tests in `test_staging_routes.py`). **Remaining:** the SA dev cluster is HELD for its own pass — see #36. Original investigation note for the record follows. — Dev tools surface (`src/cvhealthcheck/web/routes/development.py` plus 14 dedicated templates, 7 orphan helpers in `shared.py`, 4 stale data files in `data/catalog/metrics/`) is queued for retirement. Investigation 2026-05-28 produced a three-tier removal plan (Tier A safe-to-delete, Tier B requires callsite updates, Tier C requires product decisions). Most consequential dependency: the production Quick HC report's tile detail links resolve to dev routes (`main.metrics_client_growth`, `main.metrics_capacity_license`). Natural sequencing: retire after ADR 0004 phase 4 (chart section type) lands, so the workspace has its own canonical chart rendering surface that tile `detail_endpoint`s can point at. Investigation details in the chat transcript; concrete surfaces include 14 templates, 7 orphan helpers, the Chart.js CDN dependency (only consumer), the dev `/security-assessment` cluster (overlaps backlog item about retiring the legacy `/security-assessment` page in the smaller-cleanups list), the `/reportsplus/*` exploration pages (HTML wraps around CLI output), and updates needed in 4 test files plus README.md.
...
25. **[RESOLVED — phase 6.5, commit `c243057`] Tile detail_endpoint resolution.** Both tiles repointed to `main.quick_hc` (the canonical workspace renders client_growth/capacity_license after phases 6/5), matching SA/LS. Done repoint-FIRST so deleting the dev metrics routes couldn't strand `url_for()` into a `BuildError`. Guard test in `tests/test_detail_endpoint_repoint.py` asserts every tile `detail_endpoint` resolves under app context and none point at a retired dev route. (Was: tiles resolved to dev routes `main.metrics_client_growth`/`main.metrics_capacity_license` at `quickhc/registry.py:248,282`.)
